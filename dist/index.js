...
var _index83 = _interopRequireDefault(__nccwpck_require__(5438));
...
;// CONCATENATED MODULE: ./src/pkgjson.js
...
function read() {
  ...
  const raw_package_json = external_fs_default().readFileSync(package_json_path, { encoding: 'utf8'});
...
function setVersion(version) {
  return new Promise((resolve, reject) => {
    ...
    ;(0,external_child_process_namespaceObject.exec)(`npm version ${version} --no-git-tag-version --prefix ${prefix}`, (error, stdout) => {
...
/* harmony default export */ const pkgjson = ({
  getPath,
  read,
  getPatch,
  getVersion,
...
;// CONCATENATED MODULE: ./src/versionassembler.js



function versionAssembler(schema, V, increasePatch) {
  const date = new Date();
  return schema
    .split(".")
    .map((schema_part) => {
      ...
      return (0,date_fns.format)(date, schema_part);
    })
    .join(".")
}
